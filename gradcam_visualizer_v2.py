"""
Microplastic GradCAM Visualizer  (v2 — Client-Data Ready)
===========================================================
Works with both the original model and the fine-tuned model.
Pass model_path= to switch between them.

Install first:
    pip install grad-cam

Usage in Colab (cell 1):
    !pip install grad-cam -q
    exec(open('gradcam_visualizer_v2.py').read())

Cell 2 — run on client image with fine-tuned model:
    result = generate_gradcam(
        image_path  = ".../personal-data/img134.jpg",
        model_path  = ".../microplastic_fasterrcnn_finetuned.pth",  # optional
        score_thresh= 0.30,
    )
    show(result)
"""
...
# ─────────────────────────────────────────────────────────────────────────────
# DEFAULT CONFIG  (overridable via function arguments)
# ─────────────────────────────────────────────────────────────────────────────
BASE_DIR             = "/content/drive/MyDrive/Colab Notebooks/Dataset/dataset"
DEFAULT_MODEL_PATH   = os.path.join(BASE_DIR, "microplastic_fasterrcnn_finetuned.pth")
...
class FasterRCNNGradCAMWrapper(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        return cls_logits[:, 1:2]   # Microplastic class scores


# ─────────────────────────────────────────────────────────────────────────────
# MODEL LOADER  (cached so we don't reload between runs in the same session)
# ─────────────────────────────────────────────────────────────────────────────
_model_cache: dict = {}
...
def load_model(model_path: str):
    if model_path not in _model_cache:
        ...
        print(f"[INFO] Using cached model: {os.path.basename(model_path)}")
...
def detect(model, image_path: str, score_thresh: float):
...
def make_collage(original, detected, gradcam_img):
    ...
    o, d, g = fit(original), fit(detected), fit(gradcam_img)
    gap     = 4
...
def generate_gradcam(
    ...
    Args:
        ...
        model_path  : path to .pth weights (original OR fine-tuned)
        output_dir  : where to save output images
        score_thresh: detection confidence threshold (0.3 is good for client data)
    ...
    print(f"[INFO] Detecting in: {os.path.basename(image_path)}")
...
if __name__ == "__main__":
    TEST_IMAGE = os.path.join(BASE_DIR, "personal-data", "img134.jpg")
